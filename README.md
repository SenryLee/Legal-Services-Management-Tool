...
| 文书起草 | 上传真实法律文书，AI 自动识别变量并转化为模板；选模板、填变量、一键生成 .docx。 |
...
| 场景 | 说明 |
| --- | --- |
| 智能收件箱 | 从文件中自动抽取业务字段，建议新建或关联记录。 |
| AI 辅助填表 | 粘贴文本或上传文件，让 AI 生成字段草稿。 |
| 文书起草 | 从真实文书中识别变量，转化为可复用模板，再填充生成新文书。 |
...
侧边栏"文书起草"入口，支持完整的文书模板化工作流：

1. **上传文书** — 选择一篇已经写好的法律文书（.docx），如起诉状、律师函、合同等。
2. **AI 识别变量** — AI 自动分析文书内容，识别出当事人姓名、案号、日期、金额等需要替换的变量。
3. **审查确认** — 用户检查 AI 识别的变量列表，可编辑、删除或手动添加，确认后保存为模板。
4. **起草新文书** — 从模板列表中选择，填写变量值（也可从案件记录自动提取），一键生成新文书。
5. **保存输出** — 生成的 .docx 文件可保存到任意位置，格式与原模板一致。

模板存储在工作区 `.legalbiz/templates/docx/` 目录，随工作区一起迁移和备份。
